...
public class Print_Traslado {
    ...
    public void generar(String traslado, String periodo, String empresa, String tienda) {
        ...
        try {
            ...
            String c_guia = "select e.fecha_envio, td.nombre as nombre_doc, e.empresa_origen, em.razon_social, em.direccion, e.serie, e.numero, e.id_almacen_origen, ald.direccion as llegada, "
                    + "al.direccion as partida from traslados_almacen as e inner join tipo_documento as td on td.id_tido = e.id_tido inner join empresas as "
                    + "em on em.ruc_empresa =  e.empresa_origen inner join almacenes as al on al.id_almacen = e.id_almacen_origen and al.empresa = e.empresa_origen  inner "
                    + "join almacenes as ald on ald.id_almacen = e.id_almacen_destino and ald.empresa = e.empresa_destino where "
                    + "e.id_traslado = '" + traslado + "' and e.periodo = '" + periodo  + "' and e.empresa_origen = '" + empresa + "' and al.nombre = '" + tienda + "'";
            System.out.println(c_guia);
            ResultSet rs = con.consulta(st, c_guia);
            if (rs.next()) {
                fecha_envio = ven.fechaformateada(rs.getString("fecha_envio"));
                documento = rs.getString("nombre_doc").toUpperCase();
                razon_empresa = rs.getString("razon_social").toUpperCase();
                ruc_empresa = rs.getString("empresa_origen").toUpperCase();
                ...
                tipo_envio = "TRASLADO ENTRE ALMACENES";
                tienda_origen = rs.getInt("id_almacen_origen");
            }
            con.cerrar(rs);
            con.cerrar(st);
        ...
        try {
            Statement st = con.conexion();
            String c_detalle = "select de.cantidad_enviada, p.descripcion, p.codigo_externo, p.precio from detalle_traslado as de inner join productos as p on p.id_producto = de.id_producto where "
                    + "de.id_traslado = '" + traslado + "' and de.periodo = '" + periodo  + "' and de.empresa = '" + empresa + "' and de.tienda = '" + tienda_origen + "'";
            ResultSet rs = con.consulta(st, c_detalle);
            while (rs.next()) {
                filas++;
                printer.printTextWrap(12 + filas, 13 + filas + 1 , 1, 4, rs.getInt("cantidad_enviada") + "");
